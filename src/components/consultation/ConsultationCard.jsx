...
const ConsultationCard = ({item}) => {
    ...
    return(
        ...
        <div className={`consultation_list_block ${blockClass}`}>
            ...
            <h4><Link to={`/consultation/${id}`} className="white_clr">{title}</Link></h4>
            ...
            <div className="ao_sec d-flex align-items-center">
                <Link to={`/consultation/${id}`} className="site_btn">AVAIL OFFER</Link>
                <Link to={`/consultation/${id}`}><img src={payIcon} alt="icon" className="img-fluid" /></Link>
            </div>
        </div>
        </>
    );
